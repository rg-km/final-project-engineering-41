...
func (u *UserRepository) FetchUserByRole(Role string) ([]User, error) {
	// TODO: You must implement this function for fetch product by name
	//beginanswer
	//var materi Materi
	rows, err := u.db.Query("SELECT * FROM users WHERE role = ?", Role)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var user User
		err := rows.Scan(&user.ID, &user.Username, &user.Email, &user.Password, &user.NoTelp, &user.Role, &user.Loggedin)
		if err != nil {
			return nil, err
		}
		users = append(users, user)
